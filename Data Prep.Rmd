...
Membuat plot untuk mengetahui juumlah korban manusia maupun rumah yang diakibatkan oleh semua bencana menurut provinsi
...
plot_agg_3.3 <- dis %>% 
  ...
  group_by(Bulan) %>% 
  summarise(sum(Meninggal, Terluka,Hilang, Rumah_Rusak, Rumah_Terendam, Fasum_Rusak))
plot_agg_3.3
```
...
plot_agg_3.4 <- dis %>% 
  select(Bulan, Meninggal, Terluka, Hilang, Rumah_Rusak, Rumah_Terendam, Fasum_Rusak) %>% 
  group_by(Bulan) %>% 
  summarise(orgmeninggal = sum(Meninggal), orgterluka = sum(Terluka), orghilang = sum(Hilang), rmhrsk = sum(Rumah_Rusak), rmhterendam = sum(Rumah_Terendam), fasrusak= sum(Fasum_Rusak))
...
```{r}
plot_agg_3.4 %>% 
  ggplot(mapping = aes(x = Bulan, y = orgmeninggal +orgterluka )) +
  geom_col(aes())
```
...
```{r}
dis_pivot %>% 
  ggplot(mapping = aes(x = Bulan, y = Total_Korban)) +
  geom_col(aes(fill = Korban_Kerusakan))
...
plot_agg_4.1 <- dis %>% 
  select(Tanggal_Kejadian, Kejadian) %>% 
  group_by(Tanggal_Kejadian, Kejadian) %>% 
  summarise(Jumlah_Kejadian = n())
plot_agg_4.1
```
...
```{r}
plot_agg_4.1 %>% 
  ggplot(mapping = aes( x =Tanggal_Kejadian , y = Jumlah_Kejadian)) +
  geom_col(aes(fill = Kejadian) , position = "dodge")
```
